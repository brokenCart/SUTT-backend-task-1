...
if __name__ == "__main__":
    ...
    while True:
        ...
        if option == "1":
            ...
            building = input("Enter the Building Name you want to filter (Default=None): ").strip()
            capacity = input("Enter Minimum Capacity (Default=0): ").strip()
            hour = input(f"Enter the free hour you want to filter [{START_HOUR}-{END_HOUR}] (Default=None): ").strip()
            ...
            try:
                ...
                continue
            system.save_data(FILENAME)
            print(f"Success: Room with number as '{room_no}' has been booked for hour '{hour}'!")
        elif option == "4":
